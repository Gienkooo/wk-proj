...
class CvTModel(nn.Module):
    def __init__(self, in_channels, num_shapes, num_colors, num_materials,
                 ...
                 tokenizer_kernel, tokenizer_stride, use_cls_token=True, **kwargs):
        super().__init__()
        ...
        stage4_depth = max(1, depth)
        self.stage4_blks = nn.Sequential(
            *[TransformerBlock(dim4, heads4, mlp_ratio, dropout) for _ in range(stage4_depth)]
        )
    ...
    def forward(self, x: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        x = self.stage4_blks(self.stage4_tok(x) + self.pos4)  # pos only at 8×8

        attn_map = torch.softmax(self.spatial_attn(x).flatten(2), dim=-1)  # (B,1,64)
        weighted = (x.flatten(2) * attn_map).sum(dim=-1)                   # (B,64)
